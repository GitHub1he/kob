...
export class GameMap extends AcGameObject {
  ...
  check_connectivity(g, sx, sy, tx, ty) { //判断(sx,sy)和(tx,ty) 在g图中能不能连通,从点a能否到达点b
  ...
  create_walls(){
    ...
    for(let i = 0 ; i < this.inner_walls_count; i++){
      for(let j = 0 ; j < 1000 ; j++){
        ...
        //保证不是出生点和已经是墙的值不被覆盖
        if (g[r][c] || g[this.rows -1 -r][this.cols -1 -c] || r == this.rows -2 && c ==1 || c == this.cols -2 && r == 1) continue;
